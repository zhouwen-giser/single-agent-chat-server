...
| `pnpm verify:v03` stops immediately | required P13 real environment is absent                                         | supply the P13 variables in `.env.example`; keep evidence below `.tmp` |

`test:e2e:fixture` is intentionally not proof of live Open WebUI or SDAR.
The v0.3 real gates actively call the configured model and fixed SDAR, enforce
an exact clean local/remote candidate, and run a guarded destructive upgrade
only against `single_agent_chat_phase4` in a named `sacs-v03-*` container. Do
not edit evidence or use a fixture to convert a blocked scenario into a pass.
