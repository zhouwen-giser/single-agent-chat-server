...
const image = process.env.CHAT_SERVER_IMAGE ?? "single-agent-chat-server:0.6.0";
...
if (config.Labels?.["org.opencontainers.image.version"] !== "0.6.0") {
  throw new Error("Container image must carry the SACS 0.6.0 OCI version");
}
process.stdout.write(
  `Container metadata gate passed: version=0.6.0, user=${config.User}, healthcheck=present.\n`,
);
